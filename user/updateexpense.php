            ...
            <div class="dj-col m9">
                <div class="dj-row-padding">
                    <div class="dj-col m12">
                        <div class="dj-card-4 dj-round dj-hover-shadow-white">
                            <div class="dj-container dj-padding dj-margin-bottom">
                                <h2 class="dj-text-orange">Update Expense </h2>
                                <?php

                                    $ex_name = $_GET['name'];
                                    $ex_amount = $_GET['amount'];
                                    $ex_date = $_GET['date'];

                                    $mySelectedExpenses="SELECT category,time,comment FROM tbl_expenses WHERE u_id='$userId' AND ename='$ex_name' AND amount='$ex_amount' AND date='$ex_date'";
                                    $mySelectedExpensesResult=mysqli_query($conn,$mySelectedExpenses);

                                    while($seletctedExpenses = mysqli_fetch_assoc($mySelectedExpensesResult)){
                                        $ex_category = $seletctedExpenses['category'];
                                        $ex_time = $seletctedExpenses['time'];
                                        $ex_comment = $seletctedExpenses['comment'];
                                    }
                                    ...
                                    <div class="dj-row-padding">
                                        <div class="dj-half">
                                            ...
                                            <input type="number" class="dj-input dj-round-large dj-premium"
                                                name="amount" value="<?php echo $ex_amount; ?>" required>
                                        </div>
                                        <div class="dj-half">
                                            ...
                                            <select class="dj-input dj-round-large dj-premium" name="category" required>
                                                ...
                                                <option value="<?php echo $ex_category; ?>" selected disabled hidden>
                                                    <?php echo $ex_category; ?>
                                                </option>
                                                <?php
                                                }
                                                ?>
                                        ...
                                        <div class="dj-row-padding">
                                            ...
                                            <input type="text" class="dj-input dj-round-large dj-premium" name="comment"
                                                value="<?php echo $ex_comment; ?>" required>
                                        </div>
                                        <button name="btnupdateexpn"
                                            class="dj-margin-top-32 dj-margin-bottom dj-button dj-orange dj-round-large">Submit</button>
                                </form>

                                <?php 
                                
                                    if(isset($_REQUEST['btnupdateexpn'])){
                                        $u_name=$_REQUEST['name'];
                                        $u_date=$_REQUEST['date'];
                                        $u_time=$_REQUEST['time'];
                                        $u_amount=$_REQUEST['amount'];
                                        $u_category=$_REQUEST['category'];
                                        $u_comment=$_REQUEST['comment'];
                                        $updateExpense="UPDATE tbl_expenses SET ename='$u_name',date='$u_date',time='$u_time',amount='$u_amount',category='$u_category',comment='$u_comment' WHERE u_id='$userId' AND ename='$ex_name' AND amount='$ex_amount' AND date='$ex_date'";
                                        if(mysqli_query($conn,$updateExpense))
                                        {
                                            echo "<script>alert('Expenses updated successfully...');
